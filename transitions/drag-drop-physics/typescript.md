# Drag & drop with physics — TypeScript / React (Pro)
...
export function useDragDrop(
  ...
  opts?: Pick<DragDropOptions, "onDrop">,
): void {
...
```tsx
function DropTarget({ src, onDrop }: { src: string; onDrop?: () => void }) {
...
export interface DragDropOptions {
  /** The draggable element (.t-drop-chip). */
  chip: HTMLElement;
  /** The target (.t-drop-zone) — every state class lands here. */
  zone: HTMLElement;
  /** The <g> inside .t-drop-puffs; ring shells are built into it per burst. */
  puffs: SVGGElement;
  /** Fires the moment the zone accepts the chip. */
  onDrop?: () => void;
}

export interface DragDropController {
  /** Detach the pointer listeners and cancel any timers still in flight. */
  destroy: () => void;
}
...
export function createDragDrop({
  ...
  puffs,
  onDrop,
}: DragDropOptions): DragDropController {
  ...
  const smokeTurb = document.querySelector<SVGFETurbulenceElement>("#t-drop-smoke feTurbulence");
  const smokeDisp = document.querySelector<SVGFEDisplacementMapElement>("#t-drop-smoke feDisplacementMap");
  const smokeBlur = document.querySelector<SVGFEGaussianBlurElement>("#t-drop-smoke feGaussianBlur");
  ...
  function land(): void {
    ...
    if (!reduced.matches) {
      ...
      after(readNum("--drop-down-dur", 250) + readNum("--drop-wave-delay", 0), () => {
    ...
    revertTimer = after(readNum("--drop-hold", 1800), () => {
      zone.classList.add("is-emptying");
      after(readNum("--drop-out-dur", 400) + 50, () => {
        ...
        after(readNum("--drop-respawn-dur", 250) + 50, () => {
  ...
  function release(e: PointerEvent): void {
    ...
    if (dropIn) {
      ...
      dx = 0; dy = 0;
      setVars(0, 0, 0, 1);
      after(readNum("--drop-return-dur", 500) + 50, () => {
...
drag writes `--dx`/`--dy` straight to the chip's style on every `pointermove`, which must
not go through a state update — a re-render per frame would break the 1:1 tracking.
