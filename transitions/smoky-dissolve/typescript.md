# Delete with smoky dissolve — TypeScript / React (Pro)

`useSmokyDissolve` — the smoky delete as a hook over a framework-free core. The card is
snapshotted to the canvas overlay, shredded through a scrolling noise field, blurred,
and dropped under gravity; opacity fades last (p^1.6) so the smoke stays readable. All
canvas work is plain `drawImage` (no SVG filters — they're CPU-rendered in WebKit).
Reads the `--smoky-*` tokens live. Pair with the CSS variant's styles (identical class
names).

```tsx
import { useCallback, useRef, type RefObject } from "react";

export function useSmokyDissolve(
  stageRef: RefObject<HTMLElement | null>,
  cardRef: RefObject<HTMLElement | null>,
  canvasRef: RefObject<HTMLCanvasElement | null>,
  opts?: Pick<SmokyDissolveOptions, "respawn" | "onComplete">,
): () => void {
  const ctrl = useRef<ReturnType<typeof createSmokyDissolve> | null>(null);
  return useCallback(() => {
    if (!ctrl.current) {
...
```tsx
function DeletableImage({ src, onDeleted }: { src: string; onDeleted?: () => void }) {
...
Pass `respawn: true` (default) to replay the card after the dissolve — the demo
behaviour. For a real delete keep `respawn: false` and remove the node in
`onComplete`.
...
function makeEaseSampler(raw: string): (x: number) => number {
  ...
  let c: number[] | undefined = kw[(raw || "").trim()];
  if (!c) {
    const m = (raw || "").match(
...
export interface SmokyDissolveOptions {
  /** Clipping container the smoke falls inside. */
  stage: HTMLElement;
  /** The card to shred — its <img> and corner button are snapshotted. */
  card: HTMLElement;
  /** Overlay render target positioned by the driver. */
  canvas: HTMLCanvasElement;
  /** Replay the card after the dissolve (demo behaviour). Default true. */
  respawn?: boolean;
  /** Fires when the dissolve finishes — remove the card here for a real delete. */
  onComplete?: () => void;
}

export function createSmokyDissolve({
  stage,
  card,
  canvas,
  respawn = true,
  onComplete,
}: SmokyDissolveOptions) {
  ...
  function gridAt(gArr: Float32Array, x: number, y: number): number {
  ...
  function noise2(gArr: Float32Array, px: number, py: number): number {
    return (
  ...
  card.addEventListener("animationend", function (e: AnimationEvent) {
