# Image generation placeholder — TypeScript / React (Pro)

`<ImagePlaceholder state="idle | loading | revealed">` — the three-phase placeholder as a
typed component. React does only what CSS can't: build the dot lattice once on mount,
baking each dot's tempo (`--k`), negative phase (`--delay`), and frozen-frame value
(`--v`). Every visible transition — the breathing, the cross-fade reveal — hangs off the
two state classes and lives in the stylesheet. Pair with the CSS variant's styles and
`:root` tokens (identical class names).

```tsx
import { useLayoutEffect, useRef } from "react";

export type ImgenState = "idle" | "loading" | "revealed";

export interface ImagePlaceholderProps {
  /** Drives the two state classes. idle = frozen frame, loading = breathing, revealed = image. */
  state?: ImgenState;
  /** The finished image. Keep it mounted; it stays invisible until "revealed". */
  src?: string | null;
  alt?: string;
}
...
function buildField(field: HTMLElement): void {
  field.textContent = "";
...
const STATE_CLASS: Record<ImgenState, string> = {
  idle: "",
  loading: " is-loading",
...
function GeneratedImage({ prompt }: { prompt: string }) {
  const [state, setState] = useState<ImgenState>("loading");
  const [src, setSrc] = useState<string | null>(null);

  useEffect(() => {
    let alive = true;
    setState("loading");
    generateImage(prompt).then((url: string) => {
      const img = new Image();
      img.onload = () => {
        if (!alive) return;
        setSrc(url);
...
- The dots set only custom properties and positions, so `buildField` needs nothing
  beyond `HTMLElement` — no casts; `dot.style.setProperty` accepts `--*` names as-is.
